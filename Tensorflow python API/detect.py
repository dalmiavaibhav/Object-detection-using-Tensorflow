...
# setting path to download model zip file
...
# defining a function to convert image to numpy array
def load_image_into_numpy_array(image):
  (im_width, im_height) = image.size
  return np.array(image.getdata()).reshape(
      (im_height, im_width, 3)).astype(np.uint8)    

#initializing and accessing webcam 
...
category_index = label_map_util.create_category_index_from_labelmap('C:\\Users\\SML\\Desktop\\me170003058\Github\\Object-detection-using-Tensorflow\\Tensorflow python API\\models\\research\\object_detection\\data\\mscoco_label_map.pbtxt', use_display_name=True)
...
with detection_graph.as_default():
  with tf.compat.v1.Session(graph=detection_graph) as sess:
    ...
    while (True):
      ...
      print("--- %s speed ms ---" % (1000*(time.time() - start_time)))
    ...
    # exit by pressing q
      cv.imshow('frame', cv.resize(image_np, (1000,700)))
